async function loadRadar(){
  ...
  try {
    ...
    digest.innerHTML = emptyState('Check docs/radar/data/dashboard.json in the repository.');
  }
}
...
function renderSota(sota){
  ...
  const items = Object.entries(benches).map(([name, b]) => '<article class="paper compact"><h3>' + escapeHtml(name) + '</h3><p>Metric: ' + escapeHtml(b.metric || '-') + ' · Entries: ' + ((b.leaderboard || []).length) + '</p></article>');
  return items.join('');
}
